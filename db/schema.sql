-- AI Twitter Aggregator — Postgres schema
-- Target: Neon (serverless Postgres). Compatible with vanilla Postgres 14+.

create extension if not exists "pgcrypto";

-- One row per produced digest. Whether posted, rejected, queued, or failed.
...
-- Forced-priority topic seeds (rarely populated in v1).
...
-- One row per LLM API call (research, draft, or review). Drives /api/internal/cost.
